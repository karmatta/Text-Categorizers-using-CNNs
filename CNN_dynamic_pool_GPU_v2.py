...
def deepnn(x):
  ...
  with tf.name_scope('reshape'):
      print('reshape x:')
      #print(x.shape)
      x_image = tf.cast(tf.reshape(x, [1, 5109, -1, 1]), dtype='float32')
  ...
  # is down to 7x7x64 feature maps -- maps this to 1024 features.
    with tf.device("/gpu:0"):
        W_fc1 = weight_variable([1278 * 64, 1024])
        b_fc1 = bias_variable([1024])

        h_pool2_flat = tf.reshape(h_poolF, [-1, 1278 * 64])
  ...
  # Map the 1024 features to 10 classes, one for each category
